...
void deadnote()
{
  for (int thisNote = 0; thisNote < (sizeof(dead_note) / sizeof(int)); thisNote++) {

    int noteDuration = 1000 / dead_durations[thisNote];//convert duration to time delay
    if (thisNote % 3 == 0) {


      tone(3, dead_note[thisNote], noteDuration);
    }
    if (thisNote % 4 == 0) {


      tone(4, dead_note[thisNote], noteDuration);
    }   if (thisNote % 5 == 0) {


      tone(5, dead_note[thisNote], noteDuration);
    }   else {


      tone(6, dead_note[thisNote], noteDuration);
    }
    int pauseBetweenNotes = noteDuration * 2.70;
    delay(pauseBetweenNotes);
    noTone(8); //stop music on pin 8
